...
def main(args):
    ...
    logging.info("-----------------------------------------------------------------------------")
    logging.info("-----------Spatiotemporal CRL with FixMatch of xBD Building Object Image Patches-----------")
...
def train(dataloader, net, criterion, optimizer, epoch, writer, device='gpu', print_freq=5):
    ...
    for i, batch in enumerate(dataloader):
        ...
        bldg_pre_weak = weak_augmentation(bldg_pre)
        bldg_pre_strong = strong_augmentation(bldg_pre)
        bldg_post_weak = weak_augmentation(bldg_post)
        bldg_post_strong = strong_augmentation(bldg_post)

        # Feed forward with weak augmentation
        _, _, z_pre_weak, z_post_weak = net(bldg_pre_weak, bldg_post_weak)
        loss_weak = net.NT_Xent_Loss_v3(z_pre_weak, z_post_weak)

        # Make predictions and get pseudo-labels using weak augmentation
        with torch.no_grad():
            preds_pre_weak = F.softmax(z_pre_weak, dim=1)
            _, pseudo_pre_labels = torch.max(preds_pre_weak, dim=1)
            preds_post_weak = F.softmax(z_post_weak, dim=1)
            _, pseudo_post_labels = torch.max(preds_post_weak, dim=1)

        # Feed forward with strong augmentation
        _, _, z_pre_strong, z_post_strong = net(bldg_pre_strong, bldg_post_strong)
        
        # Compute consistency loss
        pre_loss = F.cross_entropy(z_pre_strong, pseudo_pre_labels)
        post_loss = F.cross_entropy(z_post_strong, pseudo_post_labels)

        # Combine losses
        loss = loss_weak + pre_loss + post_loss
